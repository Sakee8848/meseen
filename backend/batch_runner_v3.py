    ...
    def run_single_simulation(self, index: int, domain: str = "hr") -> Optional[dict]:
        ...
        try:
            ...
            try:
                ...
                dialogue_templates = {
                    "招聘与人才获取": [
                        {"q": "您是想招长期员工还是临时工？", "a": "临时的，就最近忙不过来"},
                        {"q": "大概需要招多少人？", "a": "先招个三五个吧，看看情况"},
                        {"q": "是什么岗位？技术还是运营？", "a": "都需要，主要是能干活就行"}
                    ],
                    "劳动关系与合规": [
                        {"q": "这个员工来公司多久了？", "a": "大概两三年了吧"},
                        {"q": "是什么原因想让他离开？", "a": "就是工作态度不好，老是出问题"},
                        {"q": "之前有没有给过书面警告？", "a": "口头说过几次，书面的好像没有"}
                    ],
                    "薪酬福利与税务": [
                        {"q": "公司大概多少人？", "a": "现在有四五十人左右"},
                        {"q": "目前薪资结构是怎样的？", "a": "就是基本工资加绩效，比较简单"},
                        {"q": "有没有做过税务筹划？", "a": "没有，这块不太懂"}
                    ],
                    "组织发展与培训": [
                        {"q": "公司现在处于什么发展阶段？", "a": "发展挺快的，从二三十人到现在一百多人"},
                        {"q": "主要是哪方面的问题？", "a": "感觉管理跟不上，效率越来越低"},
                        {"q": "有没有考虑做系统的培训？", "a": "想过，但不知道从哪入手"}
                    ]
                }
                
                # 获取对应类别的对话模板
                templates = dialogue_templates.get(category, [
                    {"q": "您能详细说说情况吗？", "a": "就是" + novice_intent[:30] + "..."},
                    {"q": "这个问题困扰多久了？", "a": "有一段时间了，最近比较急"},
                    {"q": "之前有尝试解决过吗？", "a": "试过一些方法，效果不太好"}
                ])
                ...
                history = [
                    ...
                    },
                    {
                        "step": 2, 
                        "role": "ai", 
                        "content": f"您好！我理解您的情况。{templates[0]['q']}"
                    },
                    {
                        "step": 3, 
                        "role": "human", 
                    ...
                    },
                    {
                        "step": 4, 
                        "role": "ai", 
                        "content": f"明白了。{templates[1]['q'] if len(templates) > 1 else '还有其他情况吗？'}"
                    },
                    {
                        "step": 5, 
                        "role": "human", 
                        "content": templates[1]['a'] if len(templates) > 1 else "差不多就这些"
                    },
                    {
                        "step": 6, 
                        "role": "ai", 
                        "content": f"根据您的描述，这是典型的「{expert_term}」问题。我们可以帮您制定专业的解决方案，需要进一步咨询吗？"
                    }
                ]

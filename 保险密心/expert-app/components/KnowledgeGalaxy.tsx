...
const KnowledgeGalaxy = () => {
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      setLogsCache(Array.isArray(logsData) ? logsData : []);
      ...
      // 根节点名称动态获取
      const rootLabel = taxData?.taxonomy?.[0]?.name ? `${taxData.taxonomy[0].name.split('【')[0]} 领域` : '核心领域';

      newNodes.push({
        id: 'root',
        data: { label: rootLabel },
        position: { x: 400, y: 0 },
        style: { background: '#0f172a', color: '#fff', border: '1px solid #334155', width: 220, borderRadius: '8px', fontWeight: 'bold' },
      });

      if (taxData?.taxonomy && Array.isArray(taxData.taxonomy)) {
        taxData.taxonomy.forEach((category, catIndex) => {
          ...
          // 保护：确保 trace_records 存在
          const catRecordCount = category.trace_records
            ? Object.values(category.trace_records).reduce((sum, arr) => (sum as number) + (Array.isArray(arr) ? arr.length : 0), 0)
            : 0;

          newNodes.push({
            id: catId,
            data: { label: category.name || `分类 ${catIndex}`, recordCount: catRecordCount },
            position: { x: catNodeX, y: catNodeY },
            style: {
              background: '#1e40af',
              color: '#fff',
              border: (catRecordCount as number) > 0 ? '2px solid #10b981' : 'none',
              width: 220,
              fontWeight: '600',
              borderRadius: '6px',
            ...
            category.services.forEach((service, svcIndex) => {
              ...
              const hasTraceRecord = category.trace_records &&
                Object.keys(category.trace_records).some(key =>
                  key && service && (key.includes(service) || service.includes(key))
                );

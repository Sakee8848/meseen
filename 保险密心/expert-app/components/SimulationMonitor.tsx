...
const SimulationMonitor = () => {
  ...
  const handleStart = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8001"}/api/start`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ domain: "insurance" })
      });
  ...
  const handleNext = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8001"}/api/next`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });
    ...
    <div className="flex flex-col h-full bg-white relative">
      ...
      <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
        <div>
          ...
          <p className="text-xs text-gray-500">中台专家模式: Insurance Services</p>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center w-16 h-16 rounded-lg transition-all active:scale-95 disabled:opacity-80 ${isCompleted
            ...
            {logs.map((log, index) => (
              <div key={index} className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-center gap-2 mb-1 text-xs text-gray-400">
                  ...
                  <span className={`uppercase px-1 rounded text-[10px] ${log.role === 'human' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100'
                    }`}>{log.role}</span>
                </div>
                <div className={`p-3 rounded-lg border leading-relaxed whitespace-pre-wrap ${log.role === 'ai' ? 'bg-blue-50 border-blue-100 text-blue-900' :
